# Copyright (C) 2019 The Bedrock developers
...
# try to make console log lines short... no timestamp, short levelname, no "bedrock."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("bedrock."):
    ...
    record.name = record.name.replace("gui.qt.main_window.BedrockWindow", "gui.qt.main_window", 1)
    return record
...
def _delete_old_logs(path, *, num_files_keep: int, max_total_size: int):
    ...
    def sortkey_oldest_first(p: pathlib.PurePath):
        ...
        # - each time bedrock is launched, there will be a new basename, ordered by date
    ...
    files = sorted(
        list(pathlib.Path(path).glob("bedrock_log_*.log*")),
...
def _configure_file_logging(
    ...
    *,
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"bedrock_log_{timestamp}_{PID}.log"
...
def _process_verbosity_log_levels(verbosity):
    ...
    for filt in filters:
        ...
        if len(items) == 1:
            level = items[0]
            bedrock_logger.setLevel(level.upper())
        elif len(items) == 2:
...
#       called from run_bedrock (the main script). This is to have this run as early
#       as possible.
# Note: some users might use Bedrock as a python library and not use run_bedrock,
...
if getattr(sys, "_BEDROCK_RUNNING_VIA_RUNBEDROCK", False):
    _inmemory_startup_logs = TruncatingMemoryHandler()
    root_logger.addHandler(_inmemory_startup_logs)

# creates a logger specifically for bedrock library
...
def get_logger(name: str) -> _CustomLogger:
    ...
    return bedrock_logger.getChild(name)
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    from . import BEDROCK_VERSION
    from .constants import GIT_REPO_URL
    _logger.info(f"Bedrock version: {BEDROCK_VERSION} - https://bedrock.org - {GIT_REPO_URL}")
