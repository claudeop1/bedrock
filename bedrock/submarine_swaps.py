...
import bedrock_aionostr.key
from bedrock_aionostr.event import Event
from bedrock_aionostr.util import to_nip19
...
class SwapManager(Logger):
    ...
    def _get_public_preimage(self, swap: SwapData) -> Optional[bytes]:
        if swap.spending_txid is None:
            return None
        # note: the used Bedrock server could omit the claim tx maliciously, causing us to miss the preimage.
        #       It's an accepted tradeoff and rather unlikely that the swap counterparty manages to make us connect to
        #       their malicious Bedrock server, unless they would spin-up some form of sybil attack and the user isn't
        #       using their personal Bedrock server.
    ...
    def server_create_swap(self, request):
        ...
        if req_type == 'reversesubmarine':
            ...
            raise Exception('Deprecated API. Please upgrade your version of Bedrock')
        else:
...
class NostrTransport(SwapServerTransport):
    ...
    async def publish_offer(self, sm: 'SwapManager') -> None:
        ...
        tags = [['d', f'bedrock-swapserver-{self.NOSTR_EVENT_VERSION}'],
    ...
    async def _get_pairs_loop(self):
        ...
        query = {
            ...
            "limit": 10,
            "#d": [f"bedrock-swapserver-{self.NOSTR_EVENT_VERSION}"],
        ...
        async for event in self.relay_manager.get_events(query, single_event=False, only_stored=False):
            try:
                ...
                continue
            if tags.get('d') != f"bedrock-swapserver-{self.NOSTR_EVENT_VERSION}":
                continue
