    ...
    def __init__(
        ...
        *,
        ...
        if plugin:  # enforce "key" starts with 'plugins.<name of plugin>.'
            pkg_prefix = "bedrock.plugins."  # for internal plugins
...
class SimpleConfig(Logger):
    ...
    def __init__(self, options=None, read_user_config_function=None,
        ...
        self.user_config = {}  # for self.get in bedrock_path()
        self.path = self.bedrock_path()
    ...
    def bedrock_path_root(self):
        # Read bedrock_path from command line
        # Otherwise use the user's default data directory.
        path = self.get('bedrock_path') or self.user_dir()
    ...
    def set_chain_config_opt_based_on_android_packagename(cls, config_options: dict[str, Any]) -> None:
        ...
        for chain in constants.NETS_LIST:
            if android_pkg_name == f"org.bedrock.{chain.cli_flag()}.bedrock":
    ...
    def bedrock_path(self):
        path = self.bedrock_path_root()
        chain = self.get_selected_chain()
        if subdir := chain.datadir_subdir():
            path = os.path.join(path, subdir)
            make_dir(path, allow_symlink=False)

        self.logger.info(f"bedrock directory {path} (chain={chain.NET_NAME})")
        return path
    ...
    NETWORK_AUTO_CONNECT = ConfigVar(
        ...
        long_desc=lambda: _("If auto-connect is enabled, Bedrock will always use a server that is on the longest blockchain. "
                            "If it is disabled, you have to choose a server you want to use. Bedrock will warn you if your server is lagging."),
    )
    NETWORK_ONESERVER = ConfigVar(
        ...
        long_desc=lambda: _(
            ...
            "\n\n"
            "By default, Bedrock tries to maintain connections to ~10 servers. "
...
If this is enabled, other nodes cannot open a channel to you. Channel recovery data is encrypted, so that only your wallet can decrypt it. However, blockchain analysis will be able to tell that the transaction was probably created by Bedrock."""),
    )
...
def read_user_config(path: Optional[str]) -> Dict[str, Any]:
    """Parse and store the user config settings in bedrock.conf into user_config[]."""
    if not path:
        return {}
