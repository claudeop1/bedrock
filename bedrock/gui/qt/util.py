...
from bedrock.util import (FileImportFailed, FileExportFailed, resource_path, EventListener,
                           get_logger, UserCancelled, UserFacingException, ChoiceItem)
from bedrock.invoices import (PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING,
                               PR_UNCONFIRMED, PR_BROADCASTING, PR_BROADCAST)
from bedrock.qrreader import MissingQrDetectionLib, QrCodeResult
from bedrock.submarine_swaps import pubkey_to_rgb_color

from bedrock.gui.common_qt.util import TaskThread
...
if TYPE_CHECKING:
    ...
    from bedrock.simple_config import ConfigVarWithConfig
...
class GenericInputHandler:
    ...
    def input_file(
        ...
        fileName = getOpenFileName(
            parent=None,
            title='select file',
            # trying to open non-text things like pdfs makes bedrock freeze
...
def import_meta_gui(bedrock_window: 'BedrockWindow', title, importer, on_success):
    filter_ = "JSON (*.json);;All files (*)"
    filename = getOpenFileName(
        parent=bedrock_window,
        title=_("Open {} file").format(title),
        filter=filter_,
        config=bedrock_window.config,
    )
    if not filename:
        return
    try:
        ...
        bedrock_window.show_critical(str(e))
    else:
        bedrock_window.show_message(_("Your {} were successfully imported").format(title))
        on_success()


def export_meta_gui(bedrock_window: 'BedrockWindow', title, exporter):
    filter_ = "JSON (*.json);;All files (*)"
    filename = getSaveFileName(
        parent=bedrock_window,
        title=_("Select file to save your {}").format(title),
        filename='bedrock_{}.json'.format(title),
        filter=filter_,
        config=bedrock_window.config,
    )
    if not filename:
    ...
    try:
        ...
        bedrock_window.show_message(_("Your {0} were exported to '{1}'")
