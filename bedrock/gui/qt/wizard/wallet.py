...
from bedrock.bip32 import is_bip32_derivation, BIP32Node, normalize_bip32_derivation, xpub_type
from bedrock.daemon import Daemon
from bedrock.i18n import _
from bedrock.keystore import bip44_derivation, bip39_to_seed, purpose48_derivation, ScriptTypeNotSupported
from bedrock.plugin import run_hook, HardwarePluginLibraryUnavailable
from bedrock.storage import StorageReadWriteError
from bedrock.util import WalletFileException, get_new_wallet_name, UserFacingException, InvalidPassword
from bedrock.util import is_subpath, ChoiceItem, multisig_type, UserCancelled, standardize_path
from bedrock.wallet import wallet_types
from .wizard import QEAbstractWizard, WizardComponent
from bedrock.logging import get_logger, Logger
from bedrock import WalletStorage, mnemonic, keystore
from bedrock.wallet_db import WalletDB
from bedrock.wizard import NewWalletWizard, KeystoreWizard, WizardViewState

from bedrock.gui.common_qt.util import ignore_if_destroyed
from bedrock.gui.qt.bip39_recovery_dialog import Bip39RecoveryDialog
from bedrock.gui.qt.password_dialog import PasswordLayout, PW_NEW, MSG_ENTER_PASSWORD, PasswordLayoutForHW
from bedrock.gui.qt.seed_dialog import SeedWidget, MSG_PASSPHRASE_WARN_ISSUE4566, KeysWidget
from bedrock.gui.qt.util import (PasswordLineEdit, char_width_in_lineedit, WWLabel, InfoButton, font_height,
                                  ChoiceWidget, MessageBoxMixin, icon_path, IconLabel, read_QIcon)
from bedrock.gui.qt.plugins_dialog import PluginsDialog

if TYPE_CHECKING:
    from bedrock.simple_config import SimpleConfig
    from bedrock.plugin import Plugins, DeviceInfo
    from bedrock.gui.qt import QBedrockApplication

WIF_HELP_TEXT = (_('WIF keys are typed in Bedrock, based on script type.') + '\n\n' +
                 _('A few examples') + ':\n' +
...
class QENewWalletWizard(NewWalletWizard, QEAbstractWizard, MessageBoxMixin):
    ...
    def __init__(self, config: 'SimpleConfig', app: 'QBedrockApplication', plugins: 'Plugins', daemon: Daemon, path, *, start_viewstate=None):
    ...
    def run_split(self, wallet_path, split_data) -> None:
        msg = _(
            "The wallet '{}' contains multiple accounts, which are no longer supported since Bedrock 2.7.\n\n"
...
class WCWalletName(WalletWizardComponent, Logger):
    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(self, parent, wizard, title=_('Bedrock wallet'))
...
class WCCreateSeed(WalletWizardComponent):
    ...
    def apply(self):
        if self.seed_widget:
            ...
            self.wizard_data['seed_variant'] = 'bedrock'
...
class WCHaveSeed(WalletWizardComponent, Logger):
    ...
    def on_ready(self):
        options = ['ext', 'bedrock', 'bip39', 'slip39']
        if self.wizard_data['wallet_type'] == '2fa':
            options = ['ext', 'bedrock']
        else:
    ...
    def is_seed(self, x):
        # really only used for bedrock seeds. bip39 and slip39 are validated in SeedWidget
        ...
        if self.wizard_data['wallet_type'] == 'standard':
            ...
            # multisig?  by default, only accept modern non-2fa bedrock seeds
    ...
    def apply(self):
        ...
        if self.seed_widget.seed_type == 'bedrock':
        ...
        def scan_task():
            ...
            if not devices:
                ...
                if sys.platform == 'win32':
                    ...
                    msg += _('While this is less than ideal, it might help if you run Bedrock as Administrator.') + '\n'
                else:
