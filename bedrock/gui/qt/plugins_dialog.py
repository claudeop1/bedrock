    ...
    def do_toggle(self):
        if not self.plugins.is_available(self.name):
            msg = "\n".join([
                ...
                _('For Bedrock to recognize external packages, you need to run it from source.')
            ])
...
class PluginsDialog(WindowModalDialog, MessageBoxMixin):
    ...
    def __init__(self, config: 'SimpleConfig', plugins: 'Plugins', *, gui_object: Optional['BedrockGui'] = None):
        WindowModalDialog.__init__(self, None, _('Bedrock Plugins'))
        ...
        website_button.clicked.connect(lambda: webopen('https://plugins.bedrock.org/'))
        hbox = QHBoxLayout()
