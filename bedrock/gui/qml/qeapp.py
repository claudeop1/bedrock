...
import bedrock
from bedrock import version, constants
from bedrock.i18n import _
from bedrock.logging import Logger, get_logger
from bedrock.bip21 import BITCOIN_BIP21_URI_SCHEME, LIGHTNING_URI_SCHEME
from bedrock.base_crash_reporter import BaseCrashReporter, EarlyExceptionsQueue
from bedrock.network import Network
from bedrock.plugin import run_hook
from bedrock.gui.common_qt.util import get_font_id
from bedrock.util import profiler
from bedrock.lnurl import SUPPORTED_LNURL_SCHEMES
...
class QEAppController(BaseCrashReporter, QObject):
    ...
    def __init__(self, qeapp: 'BedrockQmlApplication', plugins: 'Plugins'):
    ...
    def doNotify(self, wallet_name, message):
        ...
        if os.name == 'nt':
            ...
            icon = os.path.join(
                os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "icons", "bedrock.png",
            )
        try:
            ...
            notification.notify('Bedrock', message, app_icon=icon, app_name='Bedrock')
        except ImportError:
...
class BedrockQmlApplication(QGuiApplication):
    ...
    def __init__(self, args, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        qmlRegisterType(QEAmount, 'org.bedrock', 1, 0, 'Amount')
        qmlRegisterType(QEBytes, 'org.bedrock', 1, 0, 'Bytes')
        qmlRegisterType(QENewWalletWizard, 'org.bedrock', 1, 0, 'QNewWalletWizard')
        qmlRegisterType(QETermsOfUseWizard, 'org.bedrock', 1, 0, 'QTermsOfUseWizard')
        qmlRegisterType(QEServerConnectWizard, 'org.bedrock', 1, 0, 'QServerConnectWizard')
        qmlRegisterType(QEFilterProxyModel, 'org.bedrock', 1, 0, 'FilterProxyModel')
        qmlRegisterType(QSortFilterProxyModel, 'org.bedrock', 1, 0, 'QSortFilterProxyModel')

        qmlRegisterType(QEWallet, 'org.bedrock', 1, 0, 'Wallet')
        qmlRegisterType(QEBitcoin, 'org.bedrock', 1, 0, 'Bitcoin')
        qmlRegisterType(QEQRParser, 'org.bedrock', 1, 0, 'QRParser')
        qmlRegisterType(QEQRScanner, 'org.bedrock', 1, 0, 'QRScanner')
        qmlRegisterType(QEFX, 'org.bedrock', 1, 0, 'FX')
        qmlRegisterType(QETxFinalizer, 'org.bedrock', 1, 0, 'TxFinalizer')
        qmlRegisterType(QEPIResolver, 'org.bedrock', 1, 0, 'PIResolver')
        qmlRegisterType(QEInvoice, 'org.bedrock', 1, 0, 'Invoice')
        qmlRegisterType(QEInvoiceParser, 'org.bedrock', 1, 0, 'InvoiceParser')
        qmlRegisterType(QEAddressDetails, 'org.bedrock', 1, 0, 'AddressDetails')
        qmlRegisterType(QETxDetails, 'org.bedrock', 1, 0, 'TxDetails')
        qmlRegisterType(QEChannelOpener, 'org.bedrock', 1, 0, 'ChannelOpener')
        qmlRegisterType(QELnPaymentDetails, 'org.bedrock', 1, 0, 'LnPaymentDetails')
        qmlRegisterType(QEChannelDetails, 'org.bedrock', 1, 0, 'ChannelDetails')
        qmlRegisterType(QESwapHelper, 'org.bedrock', 1, 0, 'SwapHelper')
        qmlRegisterType(QERequestDetails, 'org.bedrock', 1, 0, 'RequestDetails')
        qmlRegisterType(QETxRbfFeeBumper, 'org.bedrock', 1, 0, 'TxRbfFeeBumper')
        qmlRegisterType(QETxCpfpFeeBumper, 'org.bedrock', 1, 0, 'TxCpfpFeeBumper')
        qmlRegisterType(QETxCanceller, 'org.bedrock', 1, 0, 'TxCanceller')
        qmlRegisterType(QETxSweepFinalizer, 'org.bedrock', 1, 0, 'SweepFinalizer')
        qmlRegisterType(QEBip39RecoveryListModel, 'org.bedrock', 1, 0, 'Bip39RecoveryListModel')
        qmlRegisterType(FeeSlider, 'org.bedrock', 1, 0, 'FeeSlider')
        # TODO QT6: these were declared as uncreatable, but that doesn't seem to work for pyqt6
        # qmlRegisterUncreatableType(QEAmount, 'org.bedrock', 1, 0, 'Amount', 'Amount can only be used as property')
        # qmlRegisterUncreatableType(QENewWalletWizard, 'org.bedrock', 1, 0, 'QNewWalletWizard', 'QNewWalletWizard can only be used as property')
        # qmlRegisterUncreatableType(QEServerConnectWizard, 'org.bedrock', 1, 0, 'QServerConnectWizard', 'QServerConnectWizard can only be used as property')
        # qmlRegisterUncreatableType(QEFilterProxyModel, 'org.bedrock', 1, 0, 'FilterProxyModel', 'FilterProxyModel can only be used as property')
        # qmlRegisterUncreatableType(QSortFilterProxyModel, 'org.bedrock', 1, 0, 'QSortFilterProxyModel', 'QSortFilterProxyModel can only be used as property')
        ...
        self.context.setContextProperty('BUILD', {
            'bedrock_version': version.BEDROCK_VERSION,
        ...
        self.context.setContextProperty('UI_UNIT_NAME', {
            "FEERATE_SAT_PER_VBYTE": bedrock.util.UI_UNIT_NAME_FEERATE_SAT_PER_VBYTE,
            "FEERATE_SAT_PER_VB":    bedrock.util.UI_UNIT_NAME_FEERATE_SAT_PER_VB,
            "FIXED_SAT":             bedrock.util.UI_UNIT_NAME_FIXED_SAT,
            "TXSIZE_VBYTES":         bedrock.util.UI_UNIT_NAME_TXSIZE_VBYTES,
            "MEMPOOL_MB":            bedrock.util.UI_UNIT_NAME_MEMPOOL_MB,
        })
