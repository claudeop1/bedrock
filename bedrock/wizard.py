...
from bedrock.slip39 import EncryptedSeed
from bedrock.storage import WalletStorage, StorageEncryptionVersion, StorageReadWriteError
from bedrock.util import UserFacingException
from bedrock.wallet_db import WalletDB
from bedrock.bip32 import normalize_bip32_derivation, xpub_type
from bedrock import descriptor, keystore, mnemonic, bitcoin
from bedrock.mnemonic import is_any_2fa_seed_type, can_seed_have_passphrase
from bedrock.util import multisig_type
    ...
    def create_storage(self, path: str, data: dict):
        ...
        if 'keystore_type' not in data:
            ...
            if data['seed_type'] in ['old', 'standard', 'segwit']:
                self._logger.debug('creating keystore from bedrock seed')
...
class ServerConnectWizard(AbstractWizard):
    ...
    def do_configure_proxy(self, wizard_data: dict):
        ...
        if not self._daemon.network:
            self._logger.debug('not configuring proxy, bedrock config wants offline mode')
            return
