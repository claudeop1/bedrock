...
from bedrock_ecc import ecdsa_sig64_from_r_and_s, ecdsa_der_sig_from_ecdsa_sig64, ECPubkey
...
class Peer(Logger, EventListener):
    ...
    def maybe_update_fee(self, chan: Channel):
        ...
        def does_chan_fee_need_update(chan_feerate: Union[float, int]) -> Optional[bool]:
            if chan.has_anchors():
                # TODO: once package relay and bedrock servers with submitpackage are more common,
        ...
        if not chan.constraints.is_initiator:
            if constants.net is not constants.BitcoinRegtest:
                ...
                if ratio < 0.5:
                    # Note that we trust the Bedrock server about fee rates
    ...
    def get_shutdown_fee_range(self, chan, closing_tx, is_local) -> Tuple[int, dict]:
        ...
        # Note: A malicious Bedrock server can make us pay high closing fees, capped only by FEERATE_MAX_DYNAMIC
