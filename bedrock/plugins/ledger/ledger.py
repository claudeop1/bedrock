...
from bedrock import bip32, constants
from bedrock import descriptor
from bedrock.bip32 import BIP32Node, convert_bip32_intpath_to_strpath, normalize_bip32_derivation
from bedrock.bitcoin import EncodeBase58Check, is_b58_address, is_segwit_script_type, var_int
from bedrock.crypto import hash_160
...
from bedrock.transaction import PartialTransaction, Transaction, PartialTxInput
from bedrock.util import bfh, UserFacingException, versiontuple
from bedrock.wallet import Standard_Wallet

from bedrock.hw_wallet import HardwareClientBase, HW_PluginBase
from bedrock.hw_wallet.plugin import is_any_tx_output_on_change_branch, validate_op_return_output, LibraryFoundButUnusable
from bedrock.hw_wallet.plugin import HardwareClientDummy
...
class Ledger_Client_New(Ledger_Client):
    ...
    def sign_transaction(self, keystore: Hardware_KeyStore, tx: PartialTransaction, password: str):
        ...
        try:
            ...
            for input_num, (bedrock_txin, psbt_in) in enumerate(zip(tx.inputs(), psbt.inputs)):
                if bedrock_txin.is_coinbase_input():
                ...
                if utxo is None:
                    continue
                if (desc := bedrock_txin.script_descriptor) is None:
                ...
                if is_wit:
                    ...
                    if bedrock_txin.is_p2sh_segwit():
                        if wit_ver == 0:
                ...
                if multisig is not None:
                    ...
                    # Bedrock uses sortedmulti; we make sure that the array of key information is normalized in a consistent order
