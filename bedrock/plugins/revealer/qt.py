...
Secret backup plug-in for the bedrock wallet.
...
from bedrock.util import make_dir, InvalidPassword, UserCancelled
from bedrock.gui.qt.util import (read_QIcon, EnterButton, WWLabel, icon_path,
                                  internal_plugin_icon_path, WindowModalDialog, Buttons,
                                  CloseButton, OkButton, HelpButton)
from bedrock.gui.qt.qrtextedit import ScanQRTextEdit
from bedrock.gui.qt.main_window import StatusBarButton
from bedrock.gui.qt.util import read_QIcon_from_bytes, read_QPixmap_from_bytes
from bedrock.gui.common_qt.util import paintQR
...
class Plugin(RevealerPlugin):
    ...
    def __init__(self, parent, config, name):
        ...
        self.base_dir = os.path.join(config.bedrock_path(), 'revealer')
    ...
    def password_dialog(self, msg=None, parent=None):
        from bedrock.gui.qt.password_dialog import PasswordDialog
        ...
        if not calibration_sheet:
            if is_cseed: #its a secret
                ...
                painter.drawImage(((total_distance_h))+11, ((total_distance_h))+11,
                                  QImage(internal_plugin_icon_path(self.name, 'bedrockb.png')).scaledToWidth(round(2.1*total_distance_h), Qt.TransformationMode.SmoothTransformation))
