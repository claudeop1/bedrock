...
from bedrock.util import UserCancelled
from bedrock.keystore import bip39_normalize_passphrase
from bedrock.bip32 import BIP32Node, convert_bip32_strpath_to_intpath
from bedrock.logging import Logger
from bedrock.plugin import runs_in_hwd_thread
from bedrock.hw_wallet.plugin import HardwareClientBase, HardwareHandlerBase
...
class KeepKeyClientBase(HardwareClientBase, GuiMixin, Logger):
    ...
    def has_usable_connection_with_device(self):
        try:
            res = self.ping("bedrock pinging device")
            assert res == "bedrock pinging device"
    ...
    def i4b(self, x):
        if x < 0:
            # hack. workaround for https://github.com/spesmilo/bedrock/issues/7779
            x += 2 ** 32
        return pack('>I', x)
