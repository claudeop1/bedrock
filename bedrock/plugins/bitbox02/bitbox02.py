...
from bedrock.transaction import PartialTransaction, Sighash
from bedrock.wallet import Multisig_Wallet, Deterministic_Wallet
from bedrock.util import UserFacingException
from bedrock.logging import get_logger
from bedrock.plugin import Device, DeviceInfo, runs_in_hwd_thread
from bedrock.simple_config import SimpleConfig
from bedrock.storage import get_derivation_used_for_hw_device_encryption
from bedrock.bitcoin import OnchainOutputType

import bedrock.bitcoin as bitcoin
...
class BitBox02Client(HardwareClientBase):
    ...
    def fail_if_not_initialized(self) -> None:
        ...
        if not self.bitbox02_device.device_info()["initialized"]:
            raise BitBox02NotInitialized(
                "Please initialize the BitBox02 using the BitBox app first before using the BitBox02 in bedrock"
            )

    def coin_network_from_bedrock_network(self) -> int:
    ...
    def get_password_for_storage_encryption(self) -> str:
        ...
        xpub = self.bitbox02_device.bedrock_encryption_key(derivation_list)
    ...
    def get_xpub(self, bip32_path: str, xtype: str, *, display: bool = False) -> str:
        ...
        coin_network = self.coin_network_from_bedrock_network()
    ...
    def sign_transaction(
        ...
        for txin in tx.inputs():
            ...
            if tx_script_type is None:
                tx_script_type = desc.to_legacy_bedrock_script_type()
            elif tx_script_type != desc.to_legacy_bedrock_script_type():
