# ----------------------------------------------------------------------------------
# Bedrock plugin for the Digital Bitbox hardware wallet by Shift Devices AG
# digitalbitbox.com
#
...
from bedrock.crypto import sha256d, EncodeAES_bytes, DecodeAES_bytes, hmac_oneshot
from bedrock.bitcoin import public_key_to_p2pkh, usermessage_magic, verify_usermessage_with_address
from bedrock.bip32 import BIP32Node, convert_bip32_intpath_to_strpath, is_all_public_derivation
from bedrock.bip32 import normalize_bip32_derivation
...
from bedrock.util import to_string, UserCancelled, UserFacingException, bfh, ChoiceItem
from bedrock.network import Network
from bedrock.logging import get_logger
from bedrock.plugin import runs_in_hwd_thread, run_in_hwd_thread

from bedrock.hw_wallet import HW_PluginBase, HardwareClientBase, HardwareHandlerBase
from bedrock.hw_wallet.plugin import OperationCancelled
...
class DigitalBitbox_Client(HardwareClientBase):
    ...
    def check_device_dialog(self):
        ...
        if not self.setupRunning:
            self.isInitialized = True # Wallet exists. Bedrock code later checks if the device matches the wallet
    ...
    def dbb_generate_wallet(self):
        ...
        filename = ("Bedrock-" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".pdf")
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            ...
            for i, txin in enumerate(tx.inputs()):
                ...
                assert desc
                if desc.to_legacy_bedrock_script_type() != 'p2pkh':
