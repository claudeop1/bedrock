...
from bedrock.wallet import Multisig_Wallet
from bedrock.keystore import Hardware_KeyStore
from bedrock.util import ChoiceItem

from bedrock.hw_wallet.qt import QtHandlerBase, QtPluginBase
from bedrock.hw_wallet.plugin import only_hook_if_libraries_available

from bedrock.gui.qt.wizard.wallet import WCScriptAndDerivation, WCHWXPub, WCHWUninitialized, WCHWUnlock
from bedrock.gui.qt.util import WindowModalDialog, CloseButton, getOpenFileName, getSaveFileName, RichLabel
...
class Plugin(ColdcardPlugin, QtPluginBase):
    ...
    @only_hook_if_libraries_available
    @hook
    def wallet_info_buttons(self, main_window: 'BedrockWindow', dialog):
        # user is about to see the "Wallet Information" dialog
        # - add a button if multisig wallet, and a Coldcard is a cosigner.
        assert isinstance(main_window, BedrockWindow), f"{type(main_window)}"
    ...
    def import_multisig_wallet_to_cc(self, main_window: 'BedrockWindow', coldcard_keystores: Sequence[Hardware_KeyStore]):
...
class CKCCSettingsDialog(WindowModalDialog):
    ...
    def __init__(self, window: BedrockWindow, plugin, keystore):
        ...
        assert isinstance(window, BedrockWindow), f"{type(window)}"
        self.window = window
