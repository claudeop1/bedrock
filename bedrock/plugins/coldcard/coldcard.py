#
# Coldcard Bedrock plugin main code.
#
#
import os
import time
...
from bedrock.bip32 import BIP32Node, InvalidMasterKeyVersionBytes
from bedrock.i18n import _
from bedrock.plugin import Device, hook, runs_in_hwd_thread
from bedrock.keystore import Hardware_KeyStore, KeyStoreWithMPK
from bedrock.transaction import PartialTransaction
from bedrock.wallet import Standard_Wallet, Multisig_Wallet, Abstract_Wallet
from bedrock.util import bfh, versiontuple, UserFacingException
from bedrock.logging import get_logger

from bedrock.hw_wallet import HW_PluginBase, HardwareClientBase
from bedrock.hw_wallet.plugin import LibraryFoundButUnusable, only_hook_if_libraries_available
...
class CKCCClient(HardwareClientBase):
    def __init__(self, plugin, handler, dev_path, *, is_simulator=False):
        ...
        if is_simulator:
            self.dev = BedrockColdcardDevice(dev_path, encrypt=True)
        else:
            ...
            self.dev = BedrockColdcardDevice(dev=hd, encrypt=True)
    ...
    def ping_check(self):
        ...
        req = b'1234 Bedrock Plugin 4321'      # free up to 59 bytes
        try:
...
class Coldcard_KeyStore(Hardware_KeyStore):
    ...
    def _encode_txin_type(txin_type):
        # Map from Bedrock code names to our code numbers.
...
class ColdcardPlugin(HW_PluginBase):
    ...
    def export_ms_wallet(wallet: Multisig_Wallet, fp, name):
        ...
        print('# Exported from Bedrock', file=fp)
