...
1. Install Docker
    ...
    (see [#6971](https://github.com/spesmilo/bedrock/issues/6971)).
...
Bedrock Windows builds are signed with a Microsoft Authenticode™ code signing
certificate in addition to the GPG-based signatures.

The advantage of using Authenticode is that Bedrock users won't receive a
...
|--------------------------------------------------------------------------------------------------|
| Upload signatures to 'bedrock-signatures' repo, as `$version/$filename.$builder.asc`            |
...
```
pyi-archive_viewer -l bedrock-*.exe1 > f1
pyi-archive_viewer -l bedrock-*.exe2 > f2
diff f1 f2 > d
cat d
```
Then investigate manually:
```
$ pyi-archive_viewer bedrock-*.exe1
? help
```
