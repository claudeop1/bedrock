...
NAME_ROOT=bedrock
PROJECT_ROOT="$WINEPREFIX/drive_c/bedrock"
...
info "Pip installing Bedrock. This might take a long time if the project folder is large."
$WINE_PYTHON -m pip install --no-build-isolation --no-dependencies --no-warn-script-location .
# pyinstaller needs to be able to "import bedrock_ecc", for which we need libsecp256k1:
# (or could try "pip install -e" instead)
cp bedrock/libsecp256k1-*.dll "$WINEPREFIX/drive_c/python3/Lib/site-packages/bedrock_ecc/"
popd
...
BEDROCK_CMDLINE_NAME="$NAME_ROOT-$VERSION" wine "$WINE_PYHOME/scripts/pyinstaller.exe" --noconfirm --clean pyinstaller.spec
...
# $VERSION could be passed to the bedrock.nsi script, but this would require some rewriting in the script itself.
makensis -DPRODUCT_VERSION=$VERSION bedrock.nsi

cd dist
mv bedrock-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
