...
;Variables
  ...
  !define PRODUCT_NAME "Bedrock"
  !define PRODUCT_WEB_SITE "https://github.com/spesmilo/bedrock"
  !define PRODUCT_PUBLISHER "Bedrock Technologies GmbH"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/bedrock-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\bedrock\gui\icons\bedrock.ico"
...
Section
  ...
  File /r "dist\bedrock\*.*"
  File "..\..\bedrock\gui\icons\bedrock.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\bedrock-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\bedrock-${PRODUCT_VERSION}.exe" "" "$INSTDIR\bedrock-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\bedrock-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\bedrock-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin:, lightning: and lnurl LUD-17 URIs to Bedrock
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\bedrock.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\bedrock-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning" "DefaultIcon" "$\"$INSTDIR\bedrock.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\bedrock-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  WriteRegStr HKCU "Software\Classes\lnurlp" "" "URL:lnurlp Protocol"
  WriteRegStr HKCU "Software\Classes\lnurlp" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\lnurlp" "DefaultIcon" "$\"$INSTDIR\bedrock.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\lnurlp\shell\open\command" "" "$\"$INSTDIR\bedrock-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  WriteRegStr HKCU "Software\Classes\lnurlw" "" "URL:lnurlw Protocol"
  WriteRegStr HKCU "Software\Classes\lnurlw" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\lnurlw" "DefaultIcon" "$\"$INSTDIR\bedrock.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\lnurlw\shell\open\command" "" "$\"$INSTDIR\bedrock-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\bedrock.ico"
