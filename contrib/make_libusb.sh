...
#     git checkout "${LIBUSB_VERSION}^{commit}"
    ...
    cp -fpv "$here/$pkgname/libusb/.libs/$dlname" "$PROJECT_ROOT/bedrock/$TARGET_NAME" || fail "Could not copy the $pkgname binary to its destination"
    info "$TARGET_NAME has been placed in the inner 'bedrock' folder."
