...
APPDIR="$BUILDDIR/bedrock.AppDir"
...
APPIMAGE="$DISTDIR/bedrock-$VERSION-x86_64.AppImage"
...
info "installing bedrock and its dependencies."
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
...
cp "$PROJECT_ROOT/bedrock.desktop" "$APPDIR/bedrock.desktop"
cp "$PROJECT_ROOT/bedrock/gui/icons/bedrock.png" "$APPDIR/bedrock.png"
