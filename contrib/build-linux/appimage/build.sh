...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t bedrock-appimage-builder-img \
    "$CONTRIB_APPIMAGE"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
#     info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/var/tmp/bedrock_build/appimage/fresh_clone/bedrock"
...
docker run $DOCKER_RUN_FLAGS \
    --name bedrock-appimage-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/bedrock \
    --rm \
    --workdir /opt/bedrock/contrib/build-linux/appimage \
    bedrock-appimage-builder-img \
    ./make_appimage.sh
