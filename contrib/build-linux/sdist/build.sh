...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t bedrock-sdist-builder-img \
    "$CONTRIB_SDIST"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
#     info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/var/tmp/bedrock_build/sdist/fresh_clone/bedrock"
...
docker run $DOCKER_RUN_FLAGS \
    --name bedrock-sdist-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/bedrock \
    --rm \
    --workdir /opt/bedrock/contrib/build-linux/sdist \
    --env OMIT_UNCLEAN_FILES \
    bedrock-sdist-builder-img \
    ./make_sdist.sh
