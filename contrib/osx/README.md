...
This guide explains how to build Bedrock binaries for macOS systems.
...
- The [bundled version of Qt](https://github.com/spesmilo/bedrock/issues/3685) also
...
- If you want to build binaries that conform to the macOS "Gatekeeper", so as to
  ...
  See [#6128](https://github.com/spesmilo/bedrock/issues/6128).
...
- The unix username should be `vagrant`, and `bedrock` should be cloned directly
  to the user's home dir: `/Users/vagrant/bedrock`.
...
- Installing extraneous brew packages can result in build differences.
  ...
  So having a dedicated "bedrock binary builder macOS VM" is recommended.
- Make sure that you are building from a fresh clone of bedrock
...
This creates both a folder named Bedrock.app and the .dmg file (both unsigned).
...
also run the `sign_osx.sh` script:
    ...
    CODESIGN_CERT="Developer ID Application: Bedrock Technologies GmbH (L6P37P7P56)" \
...
2. Use the provided `compare_dmg` script to compare the binary you built with
   the official release binary.
    ```
    $ ./contrib/osx/compare_dmg dist/bedrock-*.dmg bedrock_dmg_official_release.dmg
    ```
...
```
$ codesign --verify --deep --strict --verbose=2 $HOME/Desktop/bedrock-4.5.8.dmg && echo "signed"
/Users/vagrant/Desktop/bedrock-4.5.8.dmg: valid on disk
/Users/vagrant/Desktop/bedrock-4.5.8.dmg: satisfies its Designated Requirement
signed
```
```
$ codesign --verify --deep --strict --verbose=1 $HOME/Desktop/Bedrock-4.5.8.app && echo "signed"
/Users/vagrant/Desktop/Bedrock-4.5.8.app: valid on disk
/Users/vagrant/Desktop/Bedrock-4.5.8.app: satisfies its Designated Requirement
signed
```

Also see `$ codesign -dvvv $HOME/Desktop/bedrock-4.5.8.dmg`

### How to check if a file is notarized?

The outer `.dmg` is NOT notarized, but the inner `.app` is notarized:
```
$ spctl -a -vvv -t install $HOME/Desktop/bedrock-4.5.8.dmg
/Users/vagrant/Desktop/bedrock-4.5.8.dmg: rejected
source=Unnotarized Developer ID
origin=Developer ID Application: Bedrock Technologies GmbH (L6P37P7P56)
```
```
$ spctl -a -vvv -t install $HOME/Desktop/Bedrock-4.5.8.app
/Users/vagrant/Desktop/Bedrock-4.5.8.app: accepted
...
Assuming you have the two unsigned outputs of `make_osx.sh` inside `~/bedrock/dist`
(e.g. `Bedrock.app` and `bedrock-4.5.4-1368-gc8db684cc-unsigned.dmg`).

In `~/bedrock` run:

`$ CODESIGN_CERT="signing_dummy" ./contrib/osx/sign_osx.sh`

After `sign_osx.sh` finished, you will have a new `*.dmg` inside `bedrock/dist`
...
`$ IS_NOTARIZED=false ./bedrock/contrib/osx/compare_dmg <unsigned executable> <self-signed executable>`
...
Error1:
```
% export CODESIGN_CERT="Developer ID Application: Bedrock Technologies GmbH (L6P37P7P56)"
% cp -f /bin/ls ./CODESIGN_TEST
% codesign -s "$CODESIGN_CERT" -f ./CODESIGN_TEST
./CODESIGN_TEST: replacing existing signature
Warning: unable to build chain to self-signed root for signer "Developer ID Application: Bedrock Technologies GmbH (L6P37P7P56)"
./CODESIGN_TEST: errSecInternalComponent
```
