...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/bedrock-macos
    cd /tmp/bedrock-macos
...
rm -rf /tmp/bedrock-macos/image > /dev/null 2>&1
mkdir /tmp/bedrock-macos/image/
cp -r $1 /tmp/bedrock-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Bedrock_uncompressed.dmg \
    /tmp/bedrock-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Bedrock_uncompressed.dmg bedrock-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Bedrock_uncompressed.dmg

echo "Done."
sha256sum bedrock-$VERSION.dmg
