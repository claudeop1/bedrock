...
# - and then uploads sigs (if SFTPUSER), else they can be submitted as PR to spesmilo/bedrock-signatures
...
# - update locale:
#     1. cd /opt/bedrock-locale && ./update.py && git push
...
# - other BUILDERS can now also try to reproduce binaries and open PRs with sigs against spesmilo/bedrock-signatures
...
# create tarball
tarball="Bedrock-$VERSION.tar.gz"
...
srctarball="Bedrock-sourceonly-$VERSION.tar.gz"
...
# appimage
appimage="bedrock-$REV-x86_64.AppImage"
...
# windows
win1="bedrock-$REV.exe"
win2="bedrock-$REV-portable.exe"
win3="bedrock-$REV-setup.exe"
...
# android
apk1="Bedrock-$VERSION-armeabi-v7a-release.apk"
apk2="Bedrock-$VERSION-arm64-v8a-release.apk"
apk3="Bedrock-$VERSION-x86_64-release.apk"
for arch in armeabi-v7a arm64-v8a x86_64
do
    apk="Bedrock-$VERSION-$arch-release.apk"
    apk_unsigned="Bedrock-$VERSION-$arch-release-unsigned.apk"
...
if [ -z "$RELEASEMANAGER" ] ; then
    ...
    if [ -z "$SSHUSER" ]; then
        ...
        BASE_URL="https://download.bedrock.org/$VERSION"
        FILES_TO_DOWNLOAD=(
            "$tarball"
            "$srctarball"
            "$appimage"
...
EOF
    ...
    if [ -z "$SSHUSER" ]; then
        info "Signing successfully, now open a pull request with your signatures to spesmilo/bedrock-signatures"
        exit 0
    else
        # upload sigs
