...
from bedrock.interface import ServerAddr
from bedrock.keystore import bip44_derivation, Hardware_KeyStore, KeyStore, BIP32_KeyStore
from bedrock.network import NetworkParameters, ProxySettings
from bedrock.plugin import Plugins, DeviceInfo, Device
from bedrock.wizard import ServerConnectWizard, NewWalletWizard, WizardViewState, KeystoreWizard
from bedrock.daemon import Daemon
from bedrock.wallet import Abstract_Wallet, Deterministic_Wallet
from bedrock import util
from bedrock import slip39
from bedrock.bip32 import KeyOriginInfo
from bedrock import keystore
...
class WizardTestCase(BedrockTestCase):
...
class KeystoreWizardTestCase(WizardTestCase):
    ...
    async def test_haveseed_bedrock(self):
        ...
        d.update({
            'seed': myseed, 'seed_type': 'segwit', 'seed_extend': False, 'seed_variant': 'bedrock',
        })
        self.assertTrue(w.is_last_view(v.view, d))
    ...
    async def test_haveseed_ext_bedrock(self):
        w, v = self._wizard_for()
        d = v.wizard_data
        myseed = '9dk'
        mypassphrase = 'abc'
        ...
        d.update({
            'seed': myseed, 'seed_type': 'segwit', 'seed_extend': True, 'seed_variant': 'bedrock',
        })
    ...
    async def test_haveseed_bedrock__mismatching_seed(self):
        """adding an unrelated seed to an xpub-only keystore should raise"""
        w, v = self._wizard_for()
        d = v.wizard_data
        d.update({
            'seed': 'abandon bike', 'seed_type': 'segwit', 'seed_extend': False, 'seed_variant': 'bedrock',
        })
        self.assertTrue(w.is_last_view(v.view, d))
        w.resolve_next(v.view, d)
        ks, ishww = w._result
    ...
    async def test_haveseed_bedrock_oldseed(self):
        w, v = self._wizard_for()
        d = v.wizard_data
        ...
        d.update({
            'seed': myseed,
            'seed_type': 'old', 'seed_extend': False, 'seed_variant': 'bedrock',
        })
        self.assertTrue(w.is_last_view(v.view, d))
        w.resolve_next(v.view, d)
        ks, ishww = w._result
    ...
    async def test_multisig(self):
        ...
        d = v.wizard_data
        d.update({
            'seed': seed2, 'seed_type': 'segwit', 'seed_extend': False, 'seed_variant': 'bedrock',
        })
        self.assertTrue(w.is_last_view(v.view, d))
        w.resolve_next(v.view, d)
        ks, ishww = w._result
...
class WalletWizardTestCase(WizardTestCase):
    ...
    async def test_set_password_and_encrypt_file(self):
        ...
        d.update({'seed': '9dk', 'seed_type': 'segwit', 'seed_extend': False, 'seed_variant': 'bedrock'})
        v = w.resolve_next(v.view, d)
    ...
    async def test_create_standard_wallet_createseed(self):
        ...
        d.update({
            'seed': '9dk', 'seed_type': 'segwit', 'seed_extend': False, 'seed_variant': 'bedrock',
        })
        v = w.resolve_next(v.view, d)
    ...
    async def test_create_standard_wallet_createseed_passphrase(self):
        ...
        d.update({
            'seed': '9dk', 'seed_type': 'segwit', 'seed_extend': True, 'seed_variant': 'bedrock',
        })
        v = w.resolve_next(v.view, d)
    ...
    async def test_create_standard_wallet_haveseed_bedrock_oldseed(self):
        w = self._wizard_for(wallet_type='standard')
        v = w._current
        d = v.wizard_data
        self.assertEqual('keystore_type', v.view)
    ...
    async def test_create_standard_wallet_haveseed_bedrock_oldseed_in_hex_format(self):
        w = self._wizard_for(wallet_type='standard')
        v = w._current
        d = v.wizard_data
        self.assertEqual('keystore_type', v.view)
    ...
    async def test_create_standard_wallet_haveseed_bedrock_oldseed_passphrase(self):
        w = self._wizard_for(wallet_type='standard')
        v = w._current
        d = v.wizard_data
        self.assertEqual('keystore_type', v.view)
        ...
        d.update({
            'seed': 'powerful random nobody notice nothing important anyway look away hidden message over',
            'seed_type': 'old', 'seed_extend': True, 'seed_variant': 'bedrock'})
        v = w.resolve_next(v.view, d)
    ...
    async def test_create_standard_wallet_haveseed_bedrock(self):
        w = self._wizard_for(wallet_type='standard')
        v = w._current
        d = v.wizard_data
        self.assertEqual('keystore_type', v.view)
    ...
    async def test_create_standard_wallet_haveseed_bedrock_passphrase(self):
        w = self._wizard_for(wallet_type='standard')
        v = w._current
        d = v.wizard_data
        self.assertEqual('keystore_type', v.view)
        ...
        d.update({'seed': '9dk', 'seed_type': 'segwit', 'seed_extend': True, 'seed_variant': 'bedrock'})
        v = w.resolve_next(v.view, d)
        self.assertEqual('have_ext', v.view)
    ...
    async def test_2fa_createseed(self):
        ...
        d.update({
            ...
            'seed_type': '2fa', 'seed_extend': False, 'seed_variant': 'bedrock',
        })
        v = w.resolve_next(v.view, d)
    ...
    async def test_2fa_haveseed_passphrase(self):
        ...
        d.update({
            'seed': 'oblige basket safe educate whale bacon celery demand novel slice various awkward',
            'seed_type': '2fa', 'seed_extend': True, 'seed_variant': 'bedrock',
        })
        v = w.resolve_next(v.view, d)
    ...
    async def test_create_multisig_wallet_2of2_createseed_cosigner2hasmasterkey(self):
        ...
        d.update({
            ...
            'seed_type': 'segwit', 'seed_extend': False, 'seed_variant': 'bedrock'})
        v = w.resolve_next(v.view, d)
        self.assertEqual('confirm_seed', v.view)
    ...
    async def test_create_multisig_wallet_3of6_haveseed_passphrase__cs2hasbip39__cs3zpub__cs4trezor__cs5seedandpassphrase__cs6zprv(self):
        ...
        d.update({
            'seed': '9dk',
            'seed_variant': 'bedrock', 'seed_type': 'segwit', 'seed_extend': True})
        v = w.resolve_next(v.view, d)
        self.assertEqual('have_ext', v.view)
