...
class TestChannelBackup(ToyServerTestCase):
    ...
    @staticmethod
    def serialize_v1_channel_backup(cb: ImportedChannelBackupStorage) -> bytes:
        vds = BCDataStream()
        vds.write_uint16(1)  # version
    ...
    async def test_request_fclose_from_pre_v2_anchor_chan_backup_deterministic_wallet(self):
        ...
        v1_blob = self.serialize_v1_channel_backup(cb_storage)
        backup_v1 = 'channel_backup:' + pw_encode_with_version_and_mac(v1_blob, alice.get_fingerprint())
        alice.lnworker.import_channel_backup(backup_v1)
        self.assertEqual(v1_blob.hex(), alice.lnworker.db.get_dict("imported_channel_backups")[chan_id.hex()])
        cb = alice.lnworker.channel_backups[chan_id]
